...
class NetworkAnalysis:
    ...
    class FlowError(Exception):
        pass
        def print_violations(self, step=-1):
            if FULL_PRINT:
                if step >= 0:
                    print(f'violation at {step}: {self}')
                else:
                    print({self})
    ...
    def __init__(self, instance: Instance, feastol: float):
        ...
        print(f"partition network in {len(self.component)} components")
    ...
    def _flow_analysis(self, inactivearcs: set, period: int, volumes: dict):
        ...
        for k, cck in self.partition.items():
            ...
            # @todo temp test
            self.check_bounds(subflow, inactivepart, period)
            flow.update(subflow)
        return flow
    ...
    def flow_analysis(self, inactivearcs: set, period: int, volumes: dict, stopatviolation: bool) -> dict:
        ...
        try:
            ...
            err.print_violations()
            return {}
        return flow

    def check_bounds(self, flows: dict, inactivearcs: set, period: int):
        """ store or raise violation when flow bounds are exceeded. """
        for arc, q in flows.items():
            if arc in inactivearcs:
                assert q == 0
            else:
