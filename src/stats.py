...
class Stat:
    ...
    basicfmt = {'realub': 2, 'ub': 2, 'lb': 2, 'gap': 1, 'cpu': 1, 'nodes': 0}
    ...
    def _basicformat(self):
        return self.modes["solve"] in {'CVX', 'EXIP', 'EXLP'}
    ...
    def fill_realcost(self, realub):
        assert not self.all['realub']
        self.all['realub'] = realub
