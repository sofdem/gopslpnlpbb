# -*- coding: utf-8 -*-
"""
Created on Fri Apr 29 09:32:06 2022
...
def bound_tight(instance, oagap, arcvals= None):
    ...
    for tau in range(0, 2):
        ...
        repeat_t=[]
        for t in range(0, len(list(instance.horizon()))-1):
                if t >=2: 
                 if all(D[j, t][0] == D[j, t-1][0] for j, tank in instance.tanks.items()):
                     if all(D[j, t][1] == D[j, t-1][1] for j, tank in instance.tanks.items()):
                         if all(D[j, t][0] == D[j, t+1][0] for j, tank in instance.tanks.items()): 
                             if all(D[j, t][1] == D[j, t+1][1] for j, tank in instance.tanks.items()):
                                 if all(junc.demand(t)==junc.demand(t-1) for j, junc in instance.junctions.items()):
                                     repeat_t.append(t)
        ...
        for (i, j), k in instance.arcs.items():
            ...
            for t in range(0, len(list(instance.horizon()))):
                if t in repeat_t:
                    bt_arc_dic= dict([((K, t), [Z[K, t-1][0] , Z[K, t-1][1] ])])
                    Z= {**Z, **bt_arc_dic}
                else:
                ...
                if t >=2: 
                 if all(D[j, t][0] == D[j, t-1][0] for j, tank in instance.tanks.items()):
                     if all(D[j, t][1] == D[j, t-1][1] for j, tank in instance.tanks.items()):
                         if all(D[j, t][0] == D[j, t+1][0] for j, tank in instance.tanks.items()): 
                             if all(D[j, t][1] == D[j, t+1][1] for j, tank in instance.tanks.items()):
                                     if all(abs(Z[j, t-1][0] - Z[j, t][0]) <= 0.02*Z[j, t][0] for j, tank in instance.tanks.items()):
                                         if all(abs(Z[j, t-1][1] - Z[j, t][1]) <= 0.02*Z[j, t][1] for j, tank in instance.tanks.items()):
        ...
        if tau < 2:
            ...
            for K, k in instance.tanks.items():
                for t_ in range(1, len(list(instance.horizon()))):
                    if t_ in repeat_t:
                        bt_h_milp= dict([((K, t_), [ D[K, t_][0],  D[K, t_][1]]) ])
                        ...
                        elif instance.name == 'Simple_Network':
